...
{
	public class PlayerSelectedUnitsIconsWidget : Widget
	{
		...
		public PlayerSelectedUnitsIconsWidget(World world, WorldRenderer worldRenderer)
		{
			this.world = world;
		...
		protected PlayerSelectedUnitsIconsWidget(PlayerSelectedUnitsIconsWidget other)
			: base(other)
		{
			world = other.world;
		...
		public override void Draw()
		{
			...
			var parentWidth = Parent.Bounds.X + Parent.Bounds.Width;
			...
			var selectedUnits = world.Selection.Actors.Where(a => a.Owner == world.LocalPlayer && a.IsInWorld && !a.IsDead).ToArray();

			var items = new List<ArmyUnit>();
			foreach (var a in selectedUnits)
			{
				items.Add(new ArmyUnit(a.Info, player) { Count = a.TraitOrDefault<IHealth>().HP / 100 });
			}
			...
			{
				var icon = unit.Icon;
				var xOffset = new int2(queueCol * (IconWidth + IconSpacing), 0);
				var yOffset = new int2(0, queueRow * (IconHeight + IconSpacing));
				var iconTopLeft = RenderOrigin + xOffset + yOffset;
			...
			foreach (var armyIcon in armyIcons)
			{
				...
				bold.DrawTextWithContrast(text, armyIcon.Bounds.Location + new float2(iconSize.X + 8, 0) - new float2(bold.Measure(text).X, bold.TopOffset),
					Color.White, Color.Black, 1);
			}
		}

		public override PlayerSelectedUnitsIconsWidget Clone()
		{
			return new PlayerSelectedUnitsIconsWidget(this);
		}
